...
public class FilterTabsView extends FrameLayout {
    ...
    public class Tab {
        ...
        public Tab(int i, String t, String e) {
            id = i;
            title = BladeConfig.tabIcons != 2 ? t : "";
            realTitle = t;
        ...
        public boolean setTitle(String newTitle) {
            newTitle = BladeConfig.tabIcons != 2 ? newTitle : "";
            realTitle = newTitle;
    ...
    public class TabView extends View {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            float textX = ((getMeasuredWidth() - tabWidth) / 2f) + currentTab.iconWidth - (BladeConfig.tabIcons == 0 ? AndroidUtilities.dp(2) : 0);
            if (animateTextX) {
            ...
            if (animateCounterEnter || counterText != null || showRemove && (isEditing || editingStartAnimationProgress != 0)) {
                ...
                int textSpace = AndroidUtilities.dp(BladeConfig.tabIcons == 2 && BladeConfig.tabStyle <= 2 ? 3 : 6);
    ...
    private final Property<FilterTabsView, Float> COLORS = new AnimationProperties.FloatProperty<FilterTabsView>("animationValue") {
        ...
        public void setValue(FilterTabsView object, float value) {
            ...
            selectorDrawable.setColor(ColorUtils.setAlphaComponent(ColorUtils.blendARGB(color1, color2, value), BladeConfig.tabStyle >= 3 ? 0x2F : 0xFF));
    ...
    public FilterTabsView(Context context) {
        ...
        listView.setSelectorType(BladeConfig.tabStyle >= 3 ? 100 : 8);
        ...
        listView.setOnItemClickListener((view, position, x, y) -> {
            ...
            if (isEditing) {
                if (position != 0 || BladeConfig.hideAllChats) {
                    int side = AndroidUtilities.dp(6);
                    if (tabView.rect.left - side < x && tabView.rect.right + side > x) {
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        ...
        if (child == listView) {
            ...
            if (indicatorWidth != 0 && BladeConfig.tabStyle != 2) {
                canvas.save();
                ...
                updateSelector();
                selectorDrawable.setBounds((int) indicatorX - (BladeConfig.tabStyle == 3 ? AndroidUtilities.dp(10) : BladeConfig.tabStyle == 4 ? AndroidUtilities.dp(12) : 0), BladeConfig.tabStyle >= 3 ? height / 2 - AndroidUtilities.dp(BladeConfig.tabStyle == 3 ? 17 : 18) : height - AndroidUtilities.dpr(4), (int) (indicatorX + indicatorWidth) + (BladeConfig.tabStyle == 3 ? AndroidUtilities.dp(10) : BladeConfig.tabStyle == 4 ? AndroidUtilities.dp(12) : 0), BladeConfig.tabStyle >= 3 ? height / 2 + AndroidUtilities.dp(BladeConfig.tabStyle == 3 ? 17 : 18) : height);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (!tabs.isEmpty()) {
            ...
            int trueTabsWidth;
            if (!BladeConfig.hideAllChats && findDefaultTab() != null)  {
    ...
    public void notifyTabCounterChanged(int id) {
        ...
        if (oldWidth != width || invalidated) {
            ...
            adapter.notifyDataSetChanged();
            allTabsWidth = 0;
            if (!BladeConfig.hideAllChats && findDefaultTab() != null) findDefaultTab().setTitle(LocaleController.getString("FilterAllChats", R.string.FilterAllChats));
...
//                notifyItemMoved(i, i + 1);
    ...
    public class TouchHelperCallback extends ItemTouchHelper.Callback {
        ...
        public int getMovementFlags(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
            if (!BladeConfig.hideAllChats && (!isEditing || (viewHolder.getAdapterPosition() == 0 && tabs.get(0).isDefault && !UserConfig.getInstance(UserConfig.selectedAccount).isPremium()))) {
        ...
        public boolean onMove(RecyclerView recyclerView, RecyclerView.ViewHolder source, RecyclerView.ViewHolder target) {
            if (!BladeConfig.hideAllChats && ((source.getAdapterPosition() == 0 || target.getAdapterPosition() == 0) && !UserConfig.getInstance(UserConfig.selectedAccount).isPremium())) {
                return false;
            }
    ...
    public void updateSelector() {
        selectorDrawable.setColor(ColorUtils.setAlphaComponent(Theme.getColor(tabLineColorKey), BladeConfig.tabStyle >= 3 ? 0x2F : 0xFF));
        float rad = AndroidUtilities.dpf2(BladeConfig.tabStyle == 3 ? 8 : BladeConfig.tabStyle == 4 ? 30 : 3);
        if (BladeConfig.tabStyle == 1 || BladeConfig.tabStyle >= 3) {
